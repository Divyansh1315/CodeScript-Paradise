...
def request(request):
    ...
    hospital_referal_requests = refernce.objects.filter(acceptance = 1)
    hospital_referal_requests = refernce.objects.exclude(user = request.user)
    hospital_referal_requests = refernce.objects.filter(refernce_hospital = user_hospital)
